...
import MenuDesktopPrivate from "../Private/MenuDesktopPrivate";

function MenuDesktop() {
  const router = useRouter();
  const dispatch = useDispatch();

  const menuOpen = useSelector(selectMenuDesktopIsVisible);

  //queries
  const { data: allBoards } = useFetchAllBoards();
      ...
      <div>
        ...
        <div className='mt-14'>
          <h4 className='ml-6 mb-[19px]'>
            PUBLIC BOARDS ({allBoards?.length || 0})
          </h4>
          ...
          {session && (
            <MenuDesktopPrivate toggleAddBoardHandler={toggleAddBoardHandler} />
          )}
        </div>
      </div>
      <div>
